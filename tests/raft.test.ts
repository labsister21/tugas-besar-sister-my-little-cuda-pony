...
describe("RaftServer", () => {
  ...
  test("should handle execute command when leader", async () => {
    jest
      .spyOn((server as any).raftNode, "getState")
      .mockReturnValue(NodeState.LEADER);
    jest
      .spyOn((server as any).raftNode, "executeCommand")
  ...
  test("should reject execute command when not leader", async () => {
    jest
      .spyOn((server as any).raftNode, "getState")
      .mockReturnValue(NodeState.FOLLOWER);
    const leaderInfo = clusterNodes[1];
    jest
      .spyOn((server as any).raftNode, "getLeaderInfo")
      .mockReturnValue(leaderInfo);
    const response = await request((server as any).app)
      .post("/execute")
      .send({ command: { type: "SET", key: "key", value: "value" } });
